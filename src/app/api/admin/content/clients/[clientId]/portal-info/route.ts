...
export async function GET(_request: NextRequest, { params }: { params: Promise<{ clientId: string }> }) {
  ...
  const [clientResult, approversResult, batchesResult] = await Promise.all([
    ...
    supabase.from('approval_batches').select('id, calendar_id, title, slug, status, published_at, approval_batch_posts(post_id)').eq('client_id', clientId).order('published_at', { ascending: false }),
  ])
